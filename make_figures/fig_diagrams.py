...
def stage_box(ax, x, y, w, h, title, lines, title_fc, body_fc="white",
              dashed=False, title_fs=7.5, body_fs=7.0):
...
def arr(ax, x1, y1, x2, y2, color="#333333", lw=1.2, ls="-", label=None,
        lab_dx=0, lab_dy=0.028, fs=7, style="-|>", mut=11, zorder=5,
...
# ---------------------------------------------------------- framework -----
def fig_framework():
    fig, ax = plt.subplots(figsize=(W2, 2.78))
    ax.axis("off")
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)

    # top row: the three optimization stages + outcome
    stage_box(ax, 0.015, 0.56, 0.20, 0.38, "STAGE 1",
              [r"Probe-seeded", r"adjoint inverse design",
               r"exact differentiable TMM", r"$360$ solver queries"], BLUE)
    stage_box(ax, 0.285, 0.56, 0.21, 0.38, "STAGE 2",
              [r"Pathwise CVaR$_{5\%}$", r"robustification",
               r"$\nabla_\theta$ through twin", r"$+$ solver jointly"], BLUE)
    stage_box(ax, 0.565, 0.56, 0.20, 0.38, "STAGE 3",
              [r"Spec-conditioned", r"correction policy",
               r"analytic policy gradients", r"(chain-GCN encoder)"], BLUE)
    stage_box(ax, 0.835, 0.30, 0.15, 0.46, "OUTCOME",
              [r"Yield-qualified", r"sensor front-end", "",
               r"$P_5$, CVaR$_{5\%}$", r"$+7.2\%$ vs. nominal"], GREEN)

    # bottom row: data -> twin ; hidden process
    stage_box(ax, 0.015, 0.06, 0.20, 0.36, "PROCESS DATA",
              [r"$400$ historical traces", r"(recipe $\rightarrow$ outcome)",
               r"in-situ monitoring"], ORANGE)
    stage_box(ax, 0.285, 0.06, 0.21, 0.36, "FabGAN TWIN",
              [r"conditional WGAN-GP", r"$+$ moment matching",
               r"$+$ tail calibration", r"differentiable sampler"], RED)
    stage_box(ax, 0.565, 0.06, 0.20, 0.36, "TRUE PROCESS",
              [r"held out from", r"all learning",
               r"$2{,}000$ MC draws", r"(evaluation only)"], GRAY,
              dashed=True)

    # flows
    arr(ax, 0.215, 0.75, 0.285, 0.75, label=r"$\theta^\star$", lw=1.4)
    arr(ax, 0.495, 0.75, 0.565, 0.75, label=r"$\theta_{rob}$", lw=1.4)
    arr(ax, 0.765, 0.75, 0.835, 0.66, lw=1.4)
    arr(ax, 0.215, 0.24, 0.285, 0.24, label="train", lab_dy=0.015, lw=1.4,
        color=ORANGE)
    arr(ax, 0.390, 0.42, 0.390, 0.56, label=r"$\tilde{\theta}\sim G_\phi(z,\theta)$",
        lab_dx=0.105, lab_dy=-0.045, lw=1.4, color=RED)
    arr(ax, 0.660, 0.42, 0.660, 0.56, color=GRAY, ls=(0, (3, 2)),
        label="evaluate", lab_dx=0.055, lab_dy=-0.045, fs=6.5)
    arr(ax, 0.765, 0.20, 0.870, 0.30, color=GRAY, ls=(0, (3, 2)))
    # backward gradient arrow (differentiability)
    arr(ax, 0.335, 0.545, 0.335, 0.435, color=BLUE, ls=(0, (1, 1.2)),
        style="<|-", label=r"$\partial J/\partial\theta$", lab_dx=-0.065,
        lab_dy=-0.045, fs=6.5)

    ax.text(0.5, 0.995, "learn what cannot be simulated  |  "
            "differentiate what can", ha="center", va="top", fontsize=8,
