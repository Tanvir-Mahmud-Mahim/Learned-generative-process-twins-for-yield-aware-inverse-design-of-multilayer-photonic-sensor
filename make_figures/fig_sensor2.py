...
def panel_a(ax):
    ...
    ax.text(0.12, 0.900, "laser\n532 nm", ha="center", va="center",
            fontsize=9.0, color="#1A5B38", fontweight="bold")
    # beam laser -> cuvette
    ...
    ax.text(LX + 0.11, 0.965, "analyte + fluorophores", fontsize=8.8,
            ha="left", va="center")
    ...
    ax.text(LX + 0.09, 0.71, "fluorescence\nemission\n($\\lambda>560$ nm)",
            fontsize=8.2, color=EMI, ha="left", va="center")
    ax.text(LX - 0.09, 0.71, "residual\nexcitation", fontsize=8.2, color=EXC,
            ha="right", va="center")
    ...
    ax.text(LX, 0.028, "SNR set by stopband leakage (yield-critical)",
            ha="center", fontsize=7.7, style="italic", color="#555555")
...
def panel_b(ax, d, n):
    ...
    ax.text(X0 + 0.045, tot + 210, "air", ha="left", va="center",
            fontsize=8.2, color="#666666", style="italic")
    # rays
    xin = (X0 + X1) / 2 - 0.11
    arrow(ax, xin, tot + 430, xin, tot + 30, "#222222", lw=1.5, mut=9)
    ax.text(xin - 0.03, tot + 510, "incident", ha="right", fontsize=8.0)
    xr = (X0 + X1) / 2 + 0.11
    arrow(ax, xr, tot + 30, xr, tot + 430, "#999999", lw=1.0, mut=8,
          ls=(0, (4, 2)))
    ax.text(xr + 0.03, tot + 510, "reflected", ha="left", fontsize=8.0,
            color="#888888")
    xt = (X0 + X1) / 2
    ...
    ax.text(bx + 0.03, tot * 0.52, "$N=20$\nlayers", ha="left",
            va="center", fontsize=7.5)
    ax.text(bx + 0.03, tot * 0.18, "%.2f $\\mu$m" % (tot / 1000), ha="left",
            va="center", fontsize=7.5)
    # callout for one layer (upper left)
    ci = 15
    ycal = float(np.sum(d[:ci]) + d[ci] / 2) * 1000
    ax.annotate("layer $i$:\n$t_i \\in [20,120]$ nm\n$n_i \\in [1.6,2.4]$",
                xy=(X0, ycal), xytext=(-0.44, tot + 40), fontsize=7.2,
                ha="left", va="center",
    ...
    cb.set_label(r"layer index $n_i(\lambda_0)$", fontsize=8.0, labelpad=2)
    cb.ax.tick_params(labelsize=5.7)
    ax.set_xlim(-0.46, 1.30)
...
def main():
    ...
    fig = plt.figure(figsize=(W2, 2.32))
    ...
    for a, s, dx in [(axa, "(a)", 0.01), (axb, "(b)", 0.00), (axc, "(c)", -0.17)]:
        a.text(dx, 1.07, s, transform=a.transAxes, fontsize=10.6,
